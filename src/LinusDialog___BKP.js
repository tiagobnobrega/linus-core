...
const LinusDialog = initArgs => {
   ...
   */
  const registerTokenizers = tokenizers => {
    tokenizers.forEach(registerTokenizer);
  };

  /**
   * Use passed handler.
   * @param handler - Object. Handler to be used
   */
  const use = handler => {
  ...
  const resolve = async (message, ctx) => {
